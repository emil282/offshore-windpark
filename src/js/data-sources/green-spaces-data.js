...
class GreenSpacesData extends DataSource {
  constructor(city, config) {
    ...
    this.largeAreaSumGoal =
      this.config.goals["green-spaces"]["large-spaces-area"] || 16;

    this.medProximity = this.config.goals["green-spaces"]["proximity-med"] || 5;
    this.nearProximity =
      this.config.goals["green-spaces"]["proximity-near"] || 3;
    this.proximityGoalPercentage =
      this.config.goals["green-spaces"]["proximity-goal-percentage"] || 0.75;
  ...
  calculateProximities() {
    ...
    const parkTileId = getTileTypeId(this.config, "park");
    const waterTileId = getTileTypeId(this.config, "water");
    const allDistances = allDistancesToTileType(this.city.map, [
      parkTileId,
      waterTileId,
    ]);
  ...
  calculateIndex() {
    ...
    this.numGreenSpaces =
      (tileTypeCount[parkTileId] || 0) + (tileTypeCount[waterTileId] || 0);
